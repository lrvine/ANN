...
ann::ann(char *train_file, char *configuration_file, double ilearnRate,
         double imomentum, double imaxEpoch, double imaxWeightForInit,
         double itargetError, int inumberLayer) {
  // set initial value
...
void inline ann::readConfiguration(char *configuration_file) {
  ...
  cfg >> numberTrainInstances >> numberTestInstances >> numberNeuronLayer1 >>
      numberNeuronLayer2 >> numberNeuronLayer3;
  cfg.close();
    ...
    for (int i = 0; i < numberTrainInstances; i++) {
      ...
      for (int j = 0; j < numberNeuronLayer2; j++) {
        ...
        for (int k = 0; k <= numberNeuronLayer1; k++) {
          valueLayer2[j] +=
              (sigmoid(inputTrainInstances[i][k]) * weightOfNetwork[0][k][j]);
        }
      ...
      for (int j = 0; j < numberNeuronLayer3; j++) {
        error +=
            pow((valueLayer3[j] - sigmoid(outputTrainInstances[i][j])), 2) / 2;
        // partial derivative with sigmoid could be simplified as
        // (Ooutput-Target)*Output*(1-Output)
        deltaGradientOfNetwork[1][j] =
            ((valueLayer3[j] - sigmoid(outputTrainInstances[i][j])) *
             valueLayer3[j] * (1 - valueLayer3[j]));
      }
      // backprogapate to layer 2
      for (int j = 0; j < numberNeuronLayer2; j++) {
        deltaGradientOfNetwork[0][j] = 0;
        for (int k = 0; k < numberNeuronLayer3; k++) {
          deltaGradientOfNetwork[0][j] +=
              (deltaGradientOfNetwork[1][k] * weightOfNetwork[1][j][k]);
        }
        deltaGradientOfNetwork[0][j] = deltaGradientOfNetwork[0][j] *
                                       valueLayer2[j] * (1 - valueLayer2[2]);
      }
      // update layer 2 to 3 weightOfNetwork
      for (int j = 0; j < numberNeuronLayer3; j++) {
        for (int k = 0; k <= numberNeuronLayer2; k++) {
          weightDeltaOfNetwork[1][k][j] =
              learnRate * valueLayer2[k] * deltaGradientOfNetwork[1][j] +
              momentum * weightDeltaOfNetwork[1][k][j];
      ...
      for (int j = 0; j < numberNeuronLayer2; j++) {
        for (int k = 0; k <= numberNeuronLayer1; k++) {
          weightDeltaOfNetwork[0][k][j] =
              learnRate * inputTrainInstances[i][k] *
                  deltaGradientOfNetwork[0][j] +
              momentum * weightDeltaOfNetwork[0][k][j];
...
void ann::doClassify(char *test_file) {
  ...
  int *realResult = new int[numberTestInstances];  // this array store the real
  ...
  int *predictionResult =
      new int[numberTestInstances];  // this array store our prediciton
...
int inline ann::doOnePrediction(double *testInput) {
  ...
  if (std::abs(valueLayer3[0] - sigmoid(1)) >
      std::abs(valueLayer3[0] - sigmoid(2))) {
    return 2;
  } else {
    return 1;
  }
...
void ann::calculateAccuracy(int *predictionResult, int *result) {
  double outputTrainInstances =
      0;  // store the number of outputTrainInstances predictions

  for (int i = 0; i < numberTestInstances;
       i++)  // count the number of outputTrainInstances predictions
  {
    if (predictionResult[i] == result[i]) outputTrainInstances++;
    std::cout << "Test instance " << i << " : predict to be "
              << predictionResult[i] << " when it is actually " << result[i]
              << std::endl;
  }

  std::cout << "Total " << numberTestInstances << " test datas have "
            << outputTrainInstances << " correct predictions" << std::endl;
